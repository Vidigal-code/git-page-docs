...
export function resolvePageHierarchy(
    ...
    const availableTypes = CONTENT_TYPES.filter((contentType) => {
        if (contentType === "source-viewer") return Boolean(currentPage.sourceViewer);
        return Boolean(currentPage[contentType]);
    });
